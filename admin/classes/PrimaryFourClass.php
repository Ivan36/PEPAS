...
class PrimaryFourClass
{
	...
	public function addStudents($post)
	{
		...
		$select_std = "SELECT id,school_name,student_name,class FROM primary_four WHERE school_name='$school_name' AND student_name='$student_name'";
		...
		$roll_query = "SELECT * FROM primary_four";
		$roll_result = database::connect()->query($roll_query);
		if ($roll_result->num_rows <= 0) {
			$id = 40201;
			$query = "INSERT INTO primary_four(id,student_name,school_name,std_no,sch_no,date_of_birth,home_address,district,gender,year)
			VALUES('$id','$student_name','$school_name','$id','$sch_no','$birth_date','$home_address','$district','$gender','$year')";
		} else {
			$id_select = "SELECT MAX(id) AS Max_Id from primary_four";
			...
			$query = "INSERT INTO primary_four(student_name,school_name,std_no,sch_no,date_of_birth,home_address,district,gender,year)
	...
	public function selectTotalStudent()
	{
		...
		$std_query = "SELECT * FROM primary_four WHERE status='active' ORDER BY school_name asc";
	...
	public function selectSingleStudent($student_id)
	{
		...
		$std_query = "SELECT * FROM primary_four WHERE id='$student_id'";
		$result = database::connect()->query($std_query);
		if ($result->num_rows > 0) {
			return $result;
		} else {
	...
	public function updateStudent($post, $student_id)
	{
		...
		$update_query = "UPDATE primary_four SET 
	...
	{
		$std_query = "SELECT student_name,std_no FROM primary_four WHERE school_name='$school_name'";
		$result = database::connect()->query($std_query);
		if ($result->num_rows > 0) {
			return $result;
		} else {
	...
	public function selectStudentResult()
	{
		...
		$rslt_query = "SELECT * FROM primary_four_results WHERE status='active'";
	...
	public function selectSingleResult($student_no)
	{
		...
		$query = "SELECT * FROM primary_four_results WHERE std_no='$student_no'";
		$result = database::connect()->query($query);
		if ($result->num_rows > 0) {
			return $result;
		} else {
	...
	public function filterResults($post)
	{
		...
		$query = "SELECT * FROM  primary_four_results WHERE school_name='$school_name' AND year='$previous' ORDER By aggregates ASC";
	...
	public function filterStudents($post)
	{
		...
		$previous = $get_year - 1;
		$query = "SELECT * FROM  primary_four where school_name='$school_name' ORDER By id ASC";
		$restlt = database::connect()->query($query);
		if ($restlt->num_rows > 0) {
			return $restlt;
		} else {
